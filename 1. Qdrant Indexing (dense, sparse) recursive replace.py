...
class HybridSearchEngine:
    ...
    def _delete_existing_file_data(self, file_path: str) -> int:
        """기존 파일의 모든 포인트를 삭제합니다."""
        try:
            # 컬렉션이 존재하지 않으면 스킵
            if not self.client.collection_exists(collection_name=COLLECTION_NAME):
                logger.info(f"📦 컬렉션 '{COLLECTION_NAME}'이 존재하지 않아 삭제 작업을 스킵합니다.")
                return 0
            ...
            existing_points = self.client.scroll(
                ...
                limit=1,  # 개수만 확인하므로 1개만 조회
                with_payload=False,
                with_vectors=False
            )
            
            deleted_count = 0
            if existing_points[0]:  # 기존 포인트가 있다면
                # 모든 기존 포인트 삭제
                ...
                if hasattr(delete_result, 'operation_id'):
                    # 정확한 삭제 개수를 얻기 위해 다시 확인
                    remaining_points = self.client.scroll(
                        collection_name=COLLECTION_NAME,
                        scroll_filter=Filter(
                            must=[
                                FieldCondition(
                    ...
                    # 삭제 전 후 차이로 계산 (정확하지 않을 수 있음)
                    deleted_count = len(existing_points[0]) if remaining_points[0] == [] else 0
                    
                logger.info(f"🗑️  기존 파일 데이터 삭제 완료: {file_path}")
                logger.info(f"     삭제된 포인트: 약 {deleted_count}개")
            else:
                logger.info(f"📄 기존 데이터 없음: {file_path}")
            
            return deleted_count
            
        except Exception as e:
            logger.error(f"❌ 기존 파일 데이터 삭제 실패: {file_path}")
            logger.error(f"   오류: {e}")
            # 삭제 실패해도 진행 (새 데이터 추가는 계속)
            return 0

    def store_document(self, file_path: str, replace_existing: bool = True) -> int:
        """
        지정된 파일을 로드하고 Qdrant에 인덱싱하는 전체 프로세스를 실행합니다.
        
        Args:
            file_path (str): 처리할 파일 경로
            replace_existing (bool): True면 기존 파일 데이터를 삭제하고 재추가, False면 추가만
        
        Returns:
            int: 새로 추가된 청크 개수
        """
        logger.info(f"📁 문서 로딩 및 인덱싱 시작: {file_path}")
        logger.info(f"🔄 중복 처리 모드: {'교체' if replace_existing else '추가'}")
        
        # 1. 기존 데이터 삭제 (replace_existing=True인 경우)
        deleted_count = 0
        if replace_existing:
            deleted_count = self._delete_existing_file_data(file_path)
        
        # 2. DOC 파일인 경우 PDF로 변환
        try:
    ...
    def _create_payload_indexes(self):
        ...
        payload_fields = {
            "self_ref": PayloadSchemaType.KEYWORD, 
            "page_no": PayloadSchemaType.INTEGER, 
            "element_type": PayloadSchemaType.KEYWORD,
            "source_file": PayloadSchemaType.KEYWORD  # source_file 인덱스 추가
        }
    ...
    def _log_results(self, stats: Dict[str, Any], original: int, groups: int, deleted_count: int = 0, replaced: bool = False):
        """결과 로깅에 삭제/교체 정보 추가"""
        ...
        # 삭제/교체 정보 추가
        if replaced and deleted_count > 0:
            logger.info(f"  - 🗑️  기존 데이터 삭제: {deleted_count}개 포인트")
        elif replaced:
            logger.info(f"  - 📄 기존 데이터: 없음 (신규 파일)")
        
        logger.info(f"  - 원본 청크: {original}개 | Group 청크: {groups}개 | 총 인덱싱: {total}개")
        logger.info("-" * 65)
        logger.info(f"  - 구조화된 청크 비율: {stats['contextualized']}/{original} ({ctx_ratio:.1%})")
        logger.info(f"  - 요소 타입 분포: {dict(stats['types'])}")
        ...
        logger.info(f"  - 검색 경로 추천: ✅ {recommendation}")
        logger.info("="*65 + "\n")

def process_directory(engine: HybridSearchEngine, directory_path: str, replace_existing: bool = True) -> Dict[str, Any]:
    """
    지정된 디렉토리의 모든 지원 파일을 재귀적으로 처리합니다.
    
    Args:
        engine: HybridSearchEngine 인스턴스
        directory_path: 처리할 디렉토리 경로
        replace_existing: True면 기존 파일 데이터를 교체, False면 추가
    """
    ...
    logger.info(f"🔄 처리 모드: {'기존 데이터 교체' if replace_existing else '기존 데이터 유지하고 추가'}")
    logger.info("-" * 80)
    ...
    for idx, file_path in enumerate(all_files, 1):
        ...
        try:
            ...
            # 문서 처리 (replace_existing 옵션 전달)
            points_count = engine.store_document(str(file_path), replace_existing=replace_existing)
            ...
            logger.info(f"   ✅ 완료: {points_count}개 포인트 {'교체' if replace_existing else '추가'}")
...
def print_processing_summary(results: Dict[str, Any]):
    ...
    logger.info(f"🔄 처리 모드: {'기존 데이터 교체' if results.get('replace_mode', True) else '기존 데이터 유지하고 추가'}")
...
if __name__ == "__main__":
    ...
    # 중복 처리 모드 설정
    REPLACE_EXISTING = True  # True: 기존 데이터 삭제 후 재추가, False: 기존 데이터 유지하고 추가
    ...
    try:
        # 디렉토리 전체 처리
        results = process_directory(engine, data_directory, replace_existing=REPLACE_EXISTING)
        ...
        if results.get("status") == "error":
            ...
            if results['processed_files'] > 0:
                mode_text = "교체" if REPLACE_EXISTING else "추가"
                logger.info(f"🎉 배치 처리 성공! 총 {results['processed_files']}개 파일에서 {results['total_points']:,}개의 포인트가 {mode_text}되었습니다.")
            else:
